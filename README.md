# 🧠 Developer Mind OS (开发者心智系统可视化速查指南)

> **从硬核程序员到卓越团队领导者。9 个标杆级职场管理、心理人性与系统思维的高密可视化卡片。**
> 
> **A high-density cheatsheet system bridging the gap between technical excellence (Hardcore Systems) and organizational leadership. Speedrun workplace dynamics, cognitive psychology, and systems thinking.**

[![GitHub Stars](https://img.shields.io/github/stars/developer-mind-OS/developer-soft-skills-guide?style=for-the-badge&color=purple)](https://github.com/developer-mind-OS/developer-soft-skills-guide)
[![Get Bundle on Gumroad](https://img.shields.io/badge/Gumroad-Get%20Bundle-orange?style=for-the-badge&logo=gumroad)](https://gumroad.com)

---

## 💡 Why This? (为什么程序员需要心智系统？)

很多优秀的研发人员在技术上无懈可击（能够手写操作系统内核、调优百万级 QPS 分布式数据库），但在职场和生活中却常常因为以下痛点而陷入瓶颈或产生严重的心理内耗：
1. **不懂如何向上沟通**：将“向上管理”误解为谄媚，在面对跨部门资源协调和老板汇报时，无法将技术价值翻译为商业成果。
2. **陷入低价值的心理内耗**：由于极度追求逻辑和确定性，在面对复杂的职场人际关系、组织冲突和不确定性时，极易陷入心理防御机制，导致精神疲惫。
3. **缺乏结构化表达与说服力**：技术思路极其清晰，但在公开演讲、跨团队会议或产品发布时，由于缺乏结构化的故事线和人性影响力技巧，无法引起听众的共鸣。

**Developer Mind OS (开发者心智系统)** 应运而生。我们延续了“硬核技术可视化海报”的极高信息密度与精美排版哲学，将经典的心理学、管理学和系统科学巨著进行解构，提炼出可落地的高密卡片与交互式沙盒，帮助程序员建立起一套清晰、高可用的“职场软实力运行系统”。

---

## 📂 Soft Skills Catalog & Interactive Sandboxes (9大心智卡片速查目录)

这里汇总了首批发布的 9 个精选主题。您可以下载 PDF 速查卡，也可以直接在浏览器中打开知识图谱和交互式心智工作坊：

| 主题类别 | 书籍/主题名称 | PDF 速查卡 (矢量) | 在线知识图谱 | 交互心智工作坊 / 沙盒 |
|:---:|:---|:---:|:---:|:---:|
| **职场管理** | 向上管理 (`Managing Up`) | [PDF 下载](./docs/managing_up/cheatsheet.pdf) | [Map 在线图谱](https://developer-mind-OS.github.io/developer-soft-skills-guide/managing_up/map.html) | [Sandbox 互动演练](https://developer-mind-OS.github.io/developer-soft-skills-guide/managing_up/sandbox.html) |
| **心理认知** | 思考，快与慢 (`Thinking, Fast and Slow`) | [PDF 下载](./docs/thinking_fast_and_slow/cheatsheet.pdf) | - | [Sandbox 偏差审计](https://developer-mind-OS.github.io/developer-soft-skills-guide/thinking_fast_and_slow/sandbox.html) |
| **个人自救** | 摆脱精神内耗 (`Mental Burnout`) | [PDF 下载](./docs/mental_burnout/cheatsheet.pdf) | - | [Sandbox 心理防线](https://developer-mind-OS.github.io/developer-soft-skills-guide/mental_burnout/sandbox.html) |
| **人性操纵** | 影响力 (`Influence`) | [PDF 下载](./docs/influence/cheatsheet.pdf) | - | [Sandbox 说服审计](https://developer-mind-OS.github.io/developer-soft-skills-guide/influence/sandbox.html) |
| **人际关系** | 亲密关系 (`Intimate Relationships`) | [PDF 下载](./docs/intimate_relationships/cheatsheet.pdf) | - | [Sandbox 依恋工作坊](https://developer-mind-OS.github.io/developer-soft-skills-guide/intimate_relationships/sandbox.html) |
| **博弈抗衡** | 权力 (`Power`) | [PDF 下载](./docs/power/cheatsheet.pdf) | [Map 在线图谱](https://developer-mind-OS.github.io/developer-soft-skills-guide/power/map.html) | [Sandbox 交互沙盒](https://developer-mind-OS.github.io/developer-soft-skills-guide/power/sandbox.html) |
| **结构沟通** | 结构表达力 (`Structured Expression`) | [PDF 下载](./docs/structured_expression/cheatsheet.pdf) | - | [Sandbox 交互演练](https://developer-mind-OS.github.io/developer-soft-skills-guide/structured_expression/sandbox.html) |
| **系统思维** | 系统之美 (`Thinking in Systems`) | [PDF 下载](./docs/thinking_in_systems/cheatsheet.pdf) | - | - |
| **底层逻辑** | 反脆弱 (`Antifragile`) | [PDF 下载](./docs/antifragile/cheatsheet.pdf) | - | [Page 1](./docs/antifragile/page_1.png) / [Page 2](./docs/antifragile/page_2.png) / [Page 3](./docs/antifragile/page_3.png) |
| **博弈抗衡** | 潜规则 (`Hidden Rules`) | [PDF 下载](./docs/hidden_rules/cheatsheet.pdf) | - | [Sandbox 互动演练](https://developer-mind-OS.github.io/developer-soft-skills-guide/hidden_rules/sandbox.html) |
| **精神成长** | 从不妥协-法拉奇传 (`Never Compromise`) | [PDF 下载](./docs/never_compromise/cheatsheet.pdf) | [Map (CN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/never_compromise/map.html) / [(EN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/never_compromise/map-en.html) | [Sandbox (CN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/never_compromise/sandbox.html) / [(EN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/never_compromise/sandbox-en.html) |
| **博弈与领导** | 维多利亚女王传 (`Queen Victoria`) | [PDF 下载](./docs/queen_victoria/cheatsheet.pdf) | [Map (CN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/queen_victoria/map.html) / [(EN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/queen_victoria/map-en.html) | [Sandbox (CN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/queen_victoria/sandbox.html) / [(EN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/queen_victoria/sandbox-en.html) |
| **心理认知** | 认知失调 (`Cognitive Dissonance`) | [PDF 下载](./docs/cognitive_dissonance/cheatsheet.pdf) | [Map (CN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/cognitive_dissonance/map.html) / [(EN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/cognitive_dissonance/map-en.html) | [Sandbox (CN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/cognitive_dissonance/sandbox.html) / [(EN)](https://developer-mind-OS.github.io/developer-soft-skills-guide/cognitive_dissonance/sandbox-en.html) |

---

## 🔧 Print & Setup Guide (使用与打印说明)

1. **工作坊在线体验**：我们建议直接通过 GitHub Pages 访问上述各个主题的 **Interactive Sandbox（交互心智工作坊）**。它们是纯前端 HTML 页面，包含互动填空、情景模拟和心智审计表单，可协助您进行每日的职场复盘和心智建模。
2. **卡片双面打印**：本仓库中提供的 PDF 采用 landscape（横向）设计，极其适合用 A4 铜版纸进行双面彩色打印，放置在工位或桌旁，作为您工作中的“应急操作指南（SOP）”。

---

## 💎 Unlock Paid Bundle (解锁商业高级包)

如果您需要获取高解析度无水印矢量 PDF（适合 A3/A2 超大画幅海报打印挂在办公室）、离线单文件 HTML 运行包、以及 **XeLaTeX + Python Compiler 完整编译脚本源码**，可前往我们的 [Gumroad 商业包链接](https://gumroad.com) 获取全量升级版。

---

## 🔗 Technical Counterpart (硬核技术系统工程)

如果您需要提升您的技术深度与架构视野，欢迎访问我们的硬核技术可视化海报项目：
👉 **[Hardcore Systems Cheatsheet (硬核系统工程可视化速查海报)](https://github.com/hardcore-systems/hardcore-systems-cheatsheet)**
*(包含 37 个标杆开源项目如 Linux, Redis, Kafka, Kubernetes, V8 等的高清速查卡与交互沙盒。)*

---

## ⚖️ License (版权声明)

本项目文本及低清 PDF 遵循 [Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International (CC BY-NC-SA 4.0)](https://creativecommons.org/licenses/by-nc-sa/4.0/) 许可协议。
* 允许社区自由共享和演绎。
* **严禁用于任何商业目的**（如转售、付费群分发、商业培训等）。如需商业授权使用请与作者联系。
